...
as the events that track filter removal can only tell us if a single filter or clear-all
...
SEARCH_FILTER_REMOVED_DETAIL AS 
    ...
    TIMESTAMP,
    CONTEXT_PAGE_URL,
    ARRAY_TO_STRING(OBJECT_KEYS(SEARCH_OPTIONS::VARIANT), ', ') AS SEARCH_OPTIONS_SIMPLIFIED, --Get the higher level categories for the search filters to account for min/max fields
    ARRAY_SIZE(OBJECT_KEYS(SEARCH_OPTIONS::VARIANT)) AS SEARCH_FILTER_COUNT, --Count the higher level categories for the search filters to account for min/max fields
    CASE 
